...
case class SZMmu(privAddrWidth: Int) extends Component {
  ...
  val fsmMmu = new StateMachine {
    ...
    val vAddr: UInt = Reg(UInt(32 bits))

    // 下一周期准备好
    io.nextReady := False

    val ready: State with EntryPoint = new State with EntryPoint {
      whenIsActive {
        when(io.vAddrValid) {
          vAddr := io.vAddr
          wData := io.wData
